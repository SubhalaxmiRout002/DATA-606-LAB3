...
```{r}
sim_calc_streak <- calc_streak(sim_basket)
sim_calc_streak
...
```{r eval=TRUE}
length(sim_calc_streak)
summary(sim_calc_streak)
sd(sim_calc_streak)
table(sim_calc_streak)
barplot(table(sim_calc_streak))
```
```
    The distribution is unimodal and right skewed. 5 is the longest streak of baskets.
    (Note: here did not set seed so longest streak varies, 
    each run this produce different streak. Some times produced 4 or 5 or 6 or 7. 
    Previous run it showed 5 so, I have written 5. This will change in every run.)
```
...
```
```
    If we run simulation of the independent shooter the distribution result of first run 
    similar with distribution result  in second run. There may be slight variation due to 
    random sample but overall distribution result should be similar. 
    In the above bar plots distribution shows almost similar.
```
...
```
```
    The distribution pattern of Kobe and Sim are similar. Both are unimodal and right skewed. 
    So we can say that hot hand model does not fit, and the shots are independent.We can
    see maximum shots at 0.
```
